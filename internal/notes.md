...
**新对话 / 换 AI 时的接手顺序（2026-08-30 立）**：① 先**完整读完本文件第 0 节全部铁律 + 第 10 节用户要求台账**，读懂再动手；② 操作步骤类问题（令牌轮换、接新 AI、出错怎么办）查 **`internal/guide.md`**；③ 站主若直接把需求丢过来、没让你读文档，**先读再干活**，不要凭常识猜站规。

请遵守：
- 所有内容、规则、偏好、决策都应**沉淀到本仓库**（本文件 / `internal/guide.md` / 站内公告 / 源码注释 / `README.md`（对外介绍））
- 不要只在对话里口头约定一件事——必须同步写入仓库
- 用户提到新规则时，立即更新本文件对应章节 + 站内公告 + 必要时同步 `README.md`（**只同步值得对外展示的介绍性内容**）
- 每次结束前确认本次所有改动已 commit + push 到 Gitee
- **铁律 · 文档分工（2026-08-29 立，2026-08-30 扩）**：① `README.md` = **对外门面**，读者是普通同学 / 访客，只写「站点是什么、有什么功能、怎么用、在线地址」，**严禁写入**站规、内部实现、令牌/密钥机制、备份与发布口径、防刷逻辑、仓库运维细节；② 本文件 `internal/notes.md` = **规则与历史**（站规、铁律、台账、更新记录、实现细节、踩坑）；③ **`internal/guide.md` = 站主操作教程**（可照做的操作步骤：令牌轮换、接入新 AI、常见故障处理），**只写操作、不写站规**，需要讲规则时指向本文件；④ README 中**不得出现**指向 `internal/` 下任何文件的链接或路径。
- **铁律 · 内部文档隐藏（2026-08-29 立）**：本文件与 `internal/guide.md` 同存放于 `internal/`（隐蔽目录），并要求——① README 不给链接、对外目录结构里不列出；② **仓库内 `class-site-backup.zip` 必须排除 `internal/`**（否则站主下载备份包即可看到全文）；③ 线上发布包（备用站点 / Pages）一律排除；④ 源码注释中只写「见内部维护文档」，**不得写出文件名或路径**；⑤ 另需理解：仓库本身是公开的，**隐藏入口只是降低被偶然发现的概率，不是安全措施**——真正的敏感信息（令牌、口令）本来就**不入库**（见敏感信息红线）。
- **铁律 · 备用站点实时同步（2026-08-29 立）**：站主已授权——**每次站点内容更新并 push 后，无需再询问，直接重新发布 WorkBuddy 备用链接**，保证备用站与主链内容实时一致。发布口径：净化版（详见第 4 节）。**主链 GitHub Pages 由 Gitee 仓库镜像自动同步（2026-08-30 站主确认），push 即生效，无需提醒站主手动同步**；本环境无法直连 GitHub（DNS / API / 网页均不通），所以主链内容**只能由站主确认**，我无法自行验证。
...
├── backup/                 # 修改前留档（zip 包 + 单文件快照，仅本地/仓库内）
├── source/                 # 早期 React 工程，仅作追溯，不参与线上
├── secrets.json.enc        # 令牌密文（OpenSSL 加密，密码仅站主知晓）
├── class-site-backup.zip   # 站点快照备份（排除 backup/ 与 internal/）
├── README.md               # 对外介绍（面向访客，不写维护细节）
└── internal/
    ├── notes.md            # 本文件：站规 / 铁律 / 运维备忘 / 历史变更台账（主要给 AI 看）
    └── guide.md            # 站主操作教程：可照做的操作步骤（给人看，2026-08-30 新建）
```

**两个内部文档的分工（别混）**：`notes.md` 写**规则与历史**（站规、铁律、踩坑、变更台账）；`guide.md` 写**操作步骤**（怎么轮换令牌、怎么接新 AI、出问题怎么办）。**站规只进 `notes.md`，`guide.md` 只放操作说明并指向 `notes.md`。**
...
- **⚠️ 推送凭据用 SSH，禁止把令牌写进 remote URL**（2026-08-29 改）：
  ...
  - 该 access token 曾以明文出现在 remote URL 中，**建议轮换**：**具体操作步骤见 `internal/guide.md` 第 2 节**（内含「先判断是否还需要 token → 不需要就直接删」的判断流程，以及删错后的补救办法）
...
- **密码提示：6 位数字**（**仅维护者本人知晓**；忘了也不要紧——里面的令牌都可以去各自平台重新生成，不影响站点运行）
- **⚠️ 死链更正（2026-08-30）**：本节原先引用的 `./scripts/decrypt-secret.sh`、`./scripts/encrypt-secret.sh`、`secrets-hint.txt` **均已随历史脱敏删除**，不要再找。现在需要解密就**让 AI 现场写 openssl 命令**，**密码由站主自己输入、不进对话、不让 AI 记录明文**
- 不要在 README、内部文档、对话中明文暴露令牌
- **令牌轮换 / 清理的操作步骤见 `internal/guide.md` 第 2 节**（一步一步照做即可）
...
**VoiceHub 侧配套**（必须保持）：
- 仓库 `server/middleware/api-0-open-cors.ts`：开放 `/api/open/*` 跨域，白名单现含 **`https://yuzikaaang.github.io`（主链）、`https://a5048c773a210b3d4-25579.app.workbuddy.link`（备用链接，2026-08-30 加入）、`https://zikang0529.gitee.io`、`http://localhost:8899`、`http://127.0.0.1:8899`**。**文件名以 `api-0-` 开头**，保证在 `api-auth.ts` 之前执行，否则浏览器预检会被 401 拦截。删除该文件则前端发券失效。
...
- [ ] Gitee → GitHub 镜像已由**云端自动同步**（一般 1~5 分钟内 GitHub Pages 刷新；本环境无法验证，由站主确认）
...
A：WorkBuddy 链接**自 2026-08-29 起已恢复为常驻备用入口**，且每次站点更新后自动重新发布，与主链内容保持一致（链接固定为 `https://a5048c773a210b3d4-25579.app.workbuddy.link/`）。2026-08-25 曾短暂取消发布，现已恢复。主链接是 GitHub Pages（`https://yuzikaaang.github.io/class-site/`），由 Gitee 镜像自动同步。**注意**：备用链接在**微信内打不开**，需复制后用系统浏览器打开。
...
| 日期 | 内容 |
|------|------|
| 2026-08-30 | ⚠️ **运维事故：站主按教程清理 SSH 公钥后，AI 立刻失去推送能力（教训入档）**：站主执行了第 2 节建议的清理（删掉 Gitee access token + 删除沙箱 SSH 公钥 `class-site-sandbox-20260829`），**随后本环境读写全部失败**：`ssh -T git@gitee.com` / `git ls-remote` / `git push --dry-run` **均返回 `git@gitee.com: Permission denied (publickey)`**。**损失评估：代码零丢失**——删除前最后一个提交 `2a4acfb`（新建 `internal/guide.md` 那笔）**已成功推送**，Gitee 上是完整最新版；只是此后 AI 的改动只能**滞留在本地沙箱**，无法推送。**根因**：本环境推送**完全依赖**那条 SSH 私钥（`~/.ssh/id_ed25519_gitee`），公钥一删，服务端不认，SSH 认证即失败——**与 access token 无关**（token 那步不影响推送）。**⚠️ 我之前写的教程有责任**：2.4 节只写了「删之前先确认当前 AI 环境还能正常推送」——**这句太弱，且没说清后果**。站主照做后必然踩坑。已重写该节：① 明确**「删掉公钥 = 正在干活的 AI 立刻失去推送能力」**并贴出实测报错；② 给出**正确顺序**（先让 AI 当场生成新密钥对并打印公钥 → 你去「删旧的 + 加新的」→ 回来验证），**顺序不能反**；③ 点明**最关键的认知——沙箱里的私钥会随会话消失**，所以「AI 能连 Gitee」**不是配一次永久有效**，**每换一次会话就要重新配一次 key**。**同时在 `guide.md` 新增 3.0 节「先选工作模式」**，因为密钥的临时性决定了授权方式只能是三选一：① 常驻授权（省事，但沙箱回收后残留，需记得删）② 用后即删（最安全，每次手动加）③ 不授权（AI 出差异、站主自己推）；并建议常驻那条 key 起个可辨识的名字、在本文件记一笔，避免下次清理误删正在用的。故障速查表补 `Permission denied (publickey)` 与「改动滞留本地没丢」两条。**应急处置**：已当场重新生成 ed25519 密钥对（新公钥 `ssh-ed25519 AAAAC3…class-site-sandbox-20260830-2123`，指纹 `SHA256:ZlyZLIWrocwkJQkcEpkTbyKF1fLavpieKWu2WwUOmXs`），**待站主加回 Gitee 后验证连通**。**⚠️ 本条记录及其后所有文档改动暂无法推送**（连接断开中），恢复后一次性提交。 |
| 2026-08-30 | 📖 **新建站主操作教程 `internal/guide.md`（用户要求，不进公告）**：用户原话「Gitee access token 轮换具体展开说说我应该怎么操作，如果后续接入其他 AI 应该怎么做……你以后再开一个文档专门教我怎么操作作为教程」。**新建 `internal/guide.md`**，定位是「**给人看的可照做操作步骤**」，与 `notes.md`（给 AI 看的规则与历史）分工，共 7 节（0~6）：① 三句话速览 ② 日常改动怎么上线（含「你只需要最后验收」的分工表）③ **Gitee 令牌轮换**（重点）④ **接入新 AI**（重点）⑤ 其他常见操作（CORS 白名单 / 读 secrets / 备份还原 / 备用链接失效）⑥ 出问题速查 ⑦ 本文件怎么维护。**令牌轮换部分的核心判断（避免站主白忙活）**：先确认 token 还有没有在用地——当前 remote 是 `git@gitee.com:...` **走 SSH**，镜像同步用的是 **GitHub** 的 token，**所以那个 Gitee token 实际已无人使用，直接删掉即可，不必新建**。文档给出 A（直接删，推荐）/ B（确实要用就重建）两条路径，并写明**权限只勾 `projects`、不要全选**、**新令牌只显示一次要立刻复制**。另外提醒**顺手删掉沙箱加的 SSH 公钥**（title `class-site-sandbox-20260829`，`https://gitee.com/profile/sshkeys`），因为私钥在沙箱里、沙箱回收后就是个没人用的多余入口；附「误删补救表」（删公钥 → 让 AI 重新生成加回；删 token → 只能重建；**仓库内容不受影响**）。**「接入新 AI」部分是这份文档最值钱的**：① 三原则——**给权限不给密码**（推代码用 SSH 公钥就够）、**先读文档再动手**、**要求 AI 区分「验过」和「没验过」**；② **权限三档表**（🟢可直接做 / 🟡做完要说明 / 🔴必须先问），可直接发给新 AI；③ **绝不给 AI 的清单**（账号密码、长期 token、点歌 API key 明文、备份解压密码）——并写明「AI 若主动索要，直接拒绝」；④ **开场白模板**，站主换 AI 时整段复制即可，内含仓库地址、双链接、必读文件清单、硬性要求（跑测试 / 两处同步 / 亮暗双模式 AA / **汇报必须分「已验证」与「需你确认」两栏**）、以及本次需求占位；⑤ **验收格式**（`✅ 已验证` + `⚠️ 需你自己确认` 两栏，附「本环境连不上 github.io，凡是主链相关的 AI 都验不了」这个具体例子）；⑥ **结束合作时回收权限**（删 SSH key、轮换 token、确认 notes.md 末条更新记录完整——**下一位 AI 全靠它接手**）。**顺带修掉 4.1 节的死链**：该节引用的 `./scripts/decrypt-secret.sh`、`./scripts/encrypt-secret.sh`、`secrets-hint.txt` **早在历史脱敏时就删除了**（本次核实确认三个都不存在），已改为「让 AI 现场写 openssl 命令、密码自己输、不进对话」，并写明**忘了密码也没事**（里面的令牌都能去各自平台重建，不影响站点运行）。**同步登记**：第 0 节加「新对话 / 换 AI 时的接手顺序」（先读铁律 → 操作查 guide.md → 站主没让读文档也要先读）；【文档分工】铁律扩为三档（README 对外 / notes.md 规则与历史 / **guide.md 操作教程**）；【内部文档隐藏】铁律把 `guide.md` 一并纳入保护；第 2 节仓库结构补上新文件与过时的 `backups/`（实为 `backup/`）；3.1 节与 4.1 节的令牌说明都加了指向教程的指针。**未动线上代码**，因此本轮不重发布备用链接。 |
| 2026-08-30 | 🐛 **【真 bug 修复】SW 会把点歌 API 响应写进缓存 → 断网时用旧歌单「抽签成功」且不报错（用户可见，无公告）**：**背景**：CORS 白名单加好后 API 请求终于成功，而 `sw.js` 的 fetch 事件**拦截所有 GET**（含跨域）——于是 `caches.put(request, copy)` 把**歌单数据也缓存进了 Cache Storage**。**危害（实测确认，非臆测）**：学生断网或 API 挂掉时，SW 的 `fetch` 失败 → 走 `.catch(() => caches.match(request))` → **命中缓存的旧歌单并返回成功** → 前端拿到数据就正常渲染「✅ 已抽中 N 首」，**既不报错、也不显示「连不上点歌平台」的降级提示**——用户拿着过期歌单抽签却毫无察觉（实测断网时抽出的仍是线上真实歌曲「第三人称 - hush！」「回到过去 - 周杰伦」，提示条 `display:none`）。**修复**：`sw.js` 的 fetch 事件开头加白名单跳过——**跨域 且（hostname 含 `dpdns.org` 或路径以 `/api/` 开头）则直接 `return`**，不进 SW、不写缓存，交由浏览器网络栈处理，失败即真失败。**跨域静态资源不受影响**：图床上的**班徽 logo 与各卡片图标**（点歌平台 / 作业查看 / 班级课表 等，域名 `s41.ax1x.com`）仍照常缓存，**保证离线可用**——所以这里**不能**一刀切跳过所有跨域请求，否则离线时整页图标全空。**缓存版本 v7 → v8**（顺带清掉用户浏览器里 v7 时期已存下的 API 缓存）。**测试**：新增 `test_sw_api_cache.py` **10 项全过**，且做了**反向验证**——临时把跳过逻辑删掉重跑，第 3/4 项立刻失败（缓存里出现 `…/api/open/songs?page=1&limit=100`，断网时仍显示「✅ 已抽中 2 首」），恢复后全过，确认问题是真的、修复是有效的。**⚠️ 测试方法上的坑（重要）**：一开始用 `page.route(abort)` 模拟失败，**测出来的是噪音**——**Playwright 的 `page.route` 拦不到 Service Worker 自己发出的 fetch**（与之前发现的坑同源），所以 SW 里的 `fetch(request)` 照样打到线上真实 API 并成功。**正确做法是用 `context.set_offline(True)` 模拟真实断网**，SW 的 fetch 才会一起失败，从而走到缓存兜底分支。 |
| 2026-08-30 | ✅ **CORS 白名单已添加并实测生效 + 主链自动同步确认（运维，不进公告）**：站主告知「主链 GitHub Pages 由 Gitee 自动同步仓库，CORS 白名单已添加」。① **CORS 实测验证（不需要浏览器，直接模拟预检即可）**——`curl -X OPTIONS <api>/songs -H "Origin: …" -H "Access-Control-Request-Method: GET" -H "Access-Control-Request-Headers: x-api-key"`：备用链接 origin → **HTTP 204 + `access-control-allow-origin: https://a5048c773a210b3d4-25579.app.workbuddy.link`**（精确回显，非通配）+ `access-control-allow-headers: Content-Type, x-api-key` + `vary: Origin` ✅；主链 `https://yuzikaaang.github.io` → 同样放行 ✅；对照组 `https://evil-example.com` → **204 但无 ACAO 头（被拒）** ✅。**三点结论**：白名单确实配好了、是精确匹配没配成 `*`、主链未受影响。**备用站上抽歌 / 歌单管理 / 点歌券现已全部可用**，前端降级提示 `#djOfflineTip` 会因请求成功**自动隐藏**——该提示是「按请求成败」触发而非按域名判断，所以**不需要改任何代码**，将来 API 挂了或换域名也会自动再出现。② **主链同步方式确认**：由 **Gitee 仓库镜像自动同步**到 GitHub，push 到 Gitee 即自动刷新 GitHub Pages，**无需站主手动操作**。`git remote -v` 只有一个 `gitee.com` remote 是**正常的**（镜像配置在 Gitee 云端，本地不可见）。**⚠️ 文档纠错（重要）**：2026-08-30 之前，第 8 节站规台账【主链 + 备用链接实时同步】第 ④ 条被误记为「本环境未登录 GitHub，**必须每次提醒用户手动同步**」——**这是错的**，导致连续多轮收尾时误催站主。已更正为「已自动化，不要再提醒」，并在第 1 节铁律、第 7 节检查清单、第 10 节 FAQ 同步修订（FAQ 里「WorkBuddy 链接已于 2026-08-25 取消发布」也是过时表述，现为常驻备用入口）。③ **本环境无法验证主链**：`curl` / WebFetch / `dig @8.8.8.8` / `api.github.com` 四种方式**全部不通**（github.io 与 workbuddy.link 在此沙箱均超时）——**主链内容只能由站主自行确认**，以后不要假装验过了。④ **剩余待办**：Gitee access token 轮换（曾明文出现在 remote URL 与对话中，见 2026-08-29 记录）。 |
| 2026-08-30 | 🎲 **歌单管理「已播放 / 已排期」默认排除 + 公示备注可自定义（用户可见，已发公告）**：① **需求 1（用户原话「在歌单管理列表已播放和已排期的默认排除不要我点」）**——此前「排除已播放 / 排除已排期」只作用于**抽签那一刻的过滤**，歌单管理列表里这些歌仍显示为正常可勾选状态，用户得一首首手动点。**现改为列表里也自动排除**：新增 `djAutoExclude()` / `djAutoExReason(s,info)`（放在 `djSaveRounds` 之后），**每次实时从本地排期历史算出，不写进 localStorage**——与手动排除（`cls_dj_excluded` / `cls_dj_excluded_req`）的区别是：排期清空或标记取消后这首歌**自动恢复可抽**，点「清空排除」也**不会误伤**它。被自动排除的行：标题划线 + 行尾标 **`🚫 已播放` / `🚫 已排期`**（不给勾选框，带 title 说明如何恢复），**判定顺序：已播放优先于已排期**（同首歌两个条件都满足时显示「已播放」）。按点歌人视图同步显示「· 自动排除 N」；状态栏改为「共 X 首 · 手动排除 A 首 / B 人 · 已播放或已排期 C 首（默认排除，不用手动点）」。**是否生效仍受面板两个开关控制**（默认都开），开关加了 `onchange="renderDjSongs(_djSongsCache)"`，改开关列表实时变。**顺手重构**：`djRun()` 里原来那段 `rounds` 遍历删掉，改调 `djAutoExclude()`，抽签与列表共用同一套判定，避免两处逻辑漂移。**⚠️ 踩坑（差点漏）**：删了 `var rounds = djLoadRounds()...` 后，djRun 末尾保存批次处的 `rounds.push(...)` 变成未定义变量，抽签直接报 `拉取失败：rounds is not defined`（**测试里靠打印抽签结果文本才发现**——只断言「不含被排除的歌」会**假阳性通过**，因为压根没抽成）。已在保存处补 `var rounds = djLoadRounds();`。**教训：删变量声明前先 grep 该变量在函数内的所有引用。**② **需求 2（用户原话「公示时间不要展示了，最后加上备注……也可以让我手动再加备注」）**——`djCopy()` 去掉 `公示时间：` 行（`抽取时间` 保留），备注统一挪到最后；新增「公示备注」输入框 `#djCopyNote`（textarea，placeholder「例如：本次点歌不随机抽人，是对段考前十的奖励」），默认文案由 `djDefaultNote()` 返回。**站主 2026-08-30 19:05 追加要求（原话「每次每人限抽一首；已排期的不重复上。 这个是默认公示备注，不要加字漏字」）：默认备注改为固定文案，逐字为「每次每人限抽一首；已排期的不重复上。」，不要加字漏字**——此前版本是按开关动态拼的（`规则：每人限一首；已播放的不再抽；已排期的不重复上。`，带「规则：」前缀、随开关增删条目、高赞必选还会追加一句），**已被站主否掉**；现改为常量 `DJ_NOTE_DEFAULT`，`djDefaultNote(){ return DJ_NOTE_DEFAULT; }` 不再读任何开关。用户改过就存 `cls_dj_copy_note` 下次沿用，旁边「↺ 默认」按钮（`djResetCopyNote()`）清记忆并恢复该固定文案。**备注合并规则**：批次备注（历史记录里填的 `last.note`）与公示备注**合并成一行**放最后：`备注：<批次备注> <公示备注>`。③ **顺带修一个漏网的按钮**：把「抽歌面板 / 歌单管理」加进全站对比度扫描视图后（采样 507 → **591**），扫出 `.btn-green`（**微信绿 `#07c160`**）白字仅 **2.38:1**——用于「📋 一键复制公示」「查看公众号」「仅这次切换」。改为 **`#0a7d42`（5.22:1）**，hover `#06ad56`→**`#09703c`（6.18:1）**。**注**：`#07c160` 在别处是**文字色**（倒计时天数、qr-head 标题）和**渐变背景**（allhol-head），那些不受影响，只改了 `.btn-green` 这个白字按钮底。改完全站 **591 个文字元素、亮暗双模式 0 项不达标** ✅。④ **另补一条暗色兜底 CSS**：`.dark .dj-ex-tag{color:#ffb08a!important}`——原有规则只写了 `.dark #djSongsMask .dj-ex-tag`，覆盖不到新增的抽歌面板「↺ 默认」按钮；歌单管理说明里的强调文字也从写死 `#c2410c`（暗色 3.02:1）改为 `.dj-ex-tag`。⑤ **测试**：新增 `test_dj_auto_exclude_and_note.py` **38 项全过**（列表自动排除标签 / 不给勾选框 / 不写进 localStorage / 关开关后恢复 / 按人视图计数 / 状态栏文案 / 抽签侧排除回归 / 公示无「公示时间」/ 备注在最后 / 默认文案 / 手动覆盖 / 持久化 / ↺ 默认 / 批次备注合并 / **默认备注逐字匹配（无「规则：」前缀、不含「已播放」、全关开关后仍不变）**）；回归 `test_dj_filter.py` 32 项、`test_dj_offline_tip.py` 20 项、全站扫描 591 元素均通过。**旧测试同步**：`test_dj_filter.py` 里状态栏断言由「1 位点歌人」改为「/ 1 人」+「不用手动点」（措辞更新）。已做修改前备份 `backup/class-site-backup-before-20260830-1839.zip`；已重打包 `class-site-backup.zip`；已重新发布备用链接。 |
| 2026-08-30 | 🎲 **公示新增自动「抽签规则」行 + 备注默认留空（用户可见，已发公告）**：用户原话「类似这种东西也可以加进去『高赞必选 3 首 + 随机 · 不限制每人首数 · 排除已播放 · 排除已排期 · 池中 7 首』，不过尽量简洁一点。备注栏就可以不要填充了，除非是特殊点歌我需要备注其余时间留空就可以了」。**这是对当天早些时候「固定文案」方案的修正**——那版的毛病是：写死的文案跟不上开关变化（关掉「排除已播放」后备注还在说排除），而动态拼接版已被否。**现在拆成两件事**：① **公示自动生成「抽签规则」行**（新函数 `djParamsLine(r, pickedLen)`，插在「抽取时间」与「本期中选」之间），格式尽量短：`高赞必选 3 + 随机 2 · 每人限一首 · 排除已播放、已排期 · 池中 7 首`——两个排除项**合并成一段**、**不带**「N 位同学」（那是抽签结果框的细节）、全关时精简到 `随机 4 · 不限每人首数 · 池中 8 首`。**数据来源**：抽签时把参数写进 `rounds[rounds.length-1].params`（top/picked/onePer/exPlayed/exSched/pool/persons），公示**优先读该批次自己的 params**——否则「抽完又改了开关再复制公示」会出现规则与歌单对不上；老批次无 params 时按当前控件兜底（此时 pool 未知，省略「池中 N 首」）。② **备注默认留空**：`DJ_NOTE_DEFAULT` 由 `'每次每人限抽一首；已排期的不重复上。'` 改为 `''`（历史文案在注释里留档备查），label 改「公示备注（选填，会加在公示最后）」，placeholder 加「选填：」前缀，按钮由「↺ 默认」改 **「✕ 清空」**（`djResetCopyNote()` 现在等价于清空 + 清 localStorage）。备注为空时公示**不输出「备注：」行**。③ **测试**：新增 `test_dj_rule_line.py` **29 项全过**（备注默认空 / 不再预填固定文案 / 按钮文案 / 清空清 localStorage / params 落库 / 四种开关组合下规则行的精确文案 / 整行 ≤60 字符 / 不带「位同学」/ 手填备注独占最后一行 / **抽完改开关后规则行不串台** / 老批次兜底 / 批次备注与规则行互不干扰）；`test_dj_auto_exclude_and_note.py` 里旧的 6 条「固定文案」断言按新行为重写 → **43 项全过**。**⚠️ 踩坑**：测试里把 `djRun(); djCopy();` 写在同一 evaluate 会拿到**上一批**的公示（djRun 是异步拉 API）→ 必须拆开并 `wait_for_timeout(1200)` 等它落库，否则假阳性。回归 `test_dj_filter.py` 32 项、`test_dj_offline_tip.py` 20 项、全站对比度扫描 **591 元素亮暗双模式 0 不达标**（新按钮沿用 `.dj-ex-tag` + `.btn-secondary`，没引入新的低对比度）。 |
...
- **【站规】主链 + 备用链接实时同步（2026-08-30 由「备用站点实时同步」升级）**：每次站点内容更新后，按序执行 ① 同步站内公告与本文件 ② `commit + push` 到 Gitee（唯一内容源）③ **重新发布 WorkBuddy 备用链接**（`publish.js --dir /workspace/.publish-class-site`，链接固定 `https://a5048c773a210b3d4-25579.app.workbuddy.link`）——**无需再询问，直接发布**。④ **主链 GitHub Pages 同步：已自动化，不要再提醒站主手动同步**（2026-08-30 站主确认）——主链由 **Gitee 仓库镜像自动同步**到 GitHub（`git remote -v` 只有 `gitee.com` 一个 remote 是**正常的**，镜像在 Gitee 云端配置，本地看不到），push 到 Gitee 后 GitHub 仓库与 Pages 自动刷新，无需人工介入、也无需本环境登录 GitHub。**⚠️ 2026-08-30 之前本条误记为「必须每次提醒用户手动同步」，导致连续多轮在收尾时误催站主——已更正。****例外**：若站主反馈主链长时间（>15 分钟）未刷新，才提示其到 Gitee 仓库「镜像管理」页面手动触发一次。
- **【运维备忘】沙箱 SSH 密钥是临时的，删公钥会让 AI 立刻断线（2026-08-30 事故后立）**：本环境推送**完全依赖** `~/.ssh/id_ed25519_gitee` 这一对密钥，**一旦站主在 Gitee 删掉对应公钥，读写立刻全部 `Permission denied (publickey)`**（与 access token 无关）。**私钥随沙箱会话消失** —— 所以① **不是配一次永久有效**，换会话就要重配；② 会话结束后留在 Gitee 上的那条公钥就是**无用残留**，应清理；③ **清理的正确顺序**：先让 AI 当场生成新密钥对并打印公钥 → 站主去 Gitee「删旧 + 加新」→ 回来验证，**顺序不能反**。**如需清库**：本文件末尾（或站主本地）记一笔当前在用的 key 名字，避免误删正在用的那条。详细的操作步骤与「常驻 / 用后即删 / 不授权」三种工作模式见 `internal/guide.md` 第 2.4 与 3.0 节。
- **【运维备忘】点歌平台 API 的 CORS 白名单（2026-08-30 记）**：线上平台是**用户自己的 fork 仓库 `yuzikaaang/VoiceHub`**，白名单硬编码在 `server/middleware/api-0-open-cors.ts` 的 `ALLOWED_ORIGINS` Set，**无环境变量可配**；当前含 4 项：`https://yuzikaaang.github.io`、`https://zikang0529.gitee.io`、`http://localhost:8899`、`http://127.0.0.1:8899`。精确匹配（协议 + 主机 + 端口，末尾无斜杠）。**新增允许来源 = 改源码 → push main → Vercel 自动部署**。备用链接 `https://a5048c773a210b3d4-25579.app.workbuddy.link` **✅ 2026-08-30 站主已添加，并经实测确认生效**（详见当日更新记录）。备用站上抽歌 / 歌单管理 / 点歌券**现已全部可用**，前端降级提示 `#djOfflineTip` 会因请求成功而**自动隐藏**（该逻辑保留不动——万一将来 API 挂了或换域名，提示会再次自动出现，不用改代码）。**降级提示是「按请求成败」触发的，不是按域名判断，所以白名单状态变了无需动前端。**
...
| 日期 | 用户要求 | 状态 |
|------|----------|------|
| 2026-08-30 | **开设独立的操作教程文档，供站主照着操作（用户明确要求）**：原话「你以后再开一个文档专门教我怎么操作作为教程」。已建 **`internal/guide.md`**——只写**可照做的操作步骤**（令牌轮换、接入新 AI、故障速查），**不写站规**（站规仍只进 `notes.md`），需要讲规则时指向 `notes.md`。此外用户要求的具体内容：① **Gitee 令牌轮换步骤要展开讲**（含「先判断还需不需要 → 不需要就直接删」）；② **后续接入其他 AI 该怎么做**（权限分档 + 开场白模板 + 验收要求 + 结束时回收权限）。已同步登记进第 0 节接手顺序、【文档分工】与【内部文档隐藏】铁律。 | 已上线 |
| 2026-08-30 | **歌单管理「已播放 / 已排期」默认排除 + 公示备注可自定义（用户明确要求）**：① 原话「在歌单管理列表已播放和已排期的默认排除不要我点」——列表里这些歌自动排除（实时算、不入库，行尾标 🚫 已播放 / 🚫 已排期，不给勾选框，判定顺序「已播放」优先），原来只有抽签时过滤、列表里还得手动点；② 原话「公示时间不要展示了，可以在最后加上备注……也可以让我手动再加备注」——`djCopy()` 去掉公示时间行，备注挪到最后，新增可编辑的 `#djCopyNote`——**默认为空**（同日站主改要求：参数改由公示里自动生成的「抽签规则」行承担，备注只在特殊点歌时手填，如「本次不随机抽人，是对段考前十的奖励」），改过记住、有「✕ 清空」可清回空，批次备注与公示备注合并一行。**同日追加**：公示新增自动生成的「抽签规则」行。 | 已上线 |
| 2026-08-30（同日修订，**取代**下面那条已被站主否掉的「固定文案」版） | **【站规】公示 = 自动生成的「抽签规则」行 + 默认留空的备注（用户明确要求）**：原话「类似这种东西也可以加进去『高赞必选 3 首 + 随机 · 不限制每人首数 · 排除已播放 · 排除已排期 · 池中 7 首』，不过尽量简洁一点。备注栏就可以不要填充了，除非是特殊点歌我需要备注其余时间留空就可以了」。**分工从此固定**：① 机器能算的参数一律进公示的「抽签规则」行，自动生成、尽量短；② 备注栏**默认留空**，只有站主手写的才出现（特殊点歌如「本次不随机抽人，是对段考前十的奖励」）。**⚠️ 不要再往备注里预填任何内容**——此前两版都被站主否掉：动态拼接版（带「规则：」前缀、随开关增删条目）与固定文案版（`每次每人限抽一首；已排期的不重复上。`，当天早些时候刚定的，同日被推翻）。`DJ_NOTE_DEFAULT` 常量保留但值为 `''`，历史文案在注释里留档，**想恢复默认值先问站主**。违反本条＝擅自改动站主对外公示口径。 | 站规·已生效 |
| 2026-08-30 | **【站规·细则】公示「抽签规则」行的写法与数据来源**：格式尽量短，例 `高赞必选 3 + 随机 2 · 每人限一首 · 排除已播放、已排期 · 池中 7 首`——两个排除项**合并成一段**；**不带**「N 位同学」（那是抽签结果框的细节）；全关时精简到 `随机 4 · 不限每人首数 · 池中 8 首`。**数据来源**：抽签时把参数写进批次记录的 `params` 字段（top/picked/onePer/exPlayed/exSched/pool/persons），公示时优先读**该批次自己的 params**——否则站主「抽完又改了开关再复制公示」会出现规则与歌单对不上；老批次无 params 时按当前控件兜底（此时 pool 未知，省略「池中 N 首」）。函数 `djParamsLine(r, pickedLen)`，位置在公示的「抽取时间」与「本期中选」之间。 | 站规·已生效 |

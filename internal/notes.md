...
| 日期 | 用户要求 | 状态 |
|------|----------|------|
| 2026-08-30 | **歌单管理「已播放 / 已排期」默认排除 + 公示备注可自定义（用户明确要求）**：① 原话「在歌单管理列表已播放和已排期的默认排除不要我点」——列表里这些歌自动排除（实时算、不入库，行尾标 🚫 已播放 / 🚫 已排期，不给勾选框，判定顺序「已播放」优先），原来只有抽签时过滤、列表里还得手动点；② 原话「公示时间不要展示了，可以在最后加上备注……也可以让我手动再加备注」——`djCopy()` 去掉公示时间行，备注挪到最后，新增可编辑的 `#djCopyNote`（默认文案为站主指定的固定句，可自己写如「本次不随机抽人，是对段考前十的奖励」），改过记住、有「↺ 默认」可恢复，批次备注与公示备注合并一行。 | 已上线 |
| 2026-08-30 | **【站规】默认公示备注为固定文案，不得增删字、不得动态生成（用户明确要求）**：原话「每次每人限抽一首；已排期的不重复上。 这个是默认公示备注，不要加字漏字」。**默认公示备注逐字固定为「每次每人限抽一首；已排期的不重复上。」**——不得按开关动态拼接、不得加「规则：」之类前缀、不得增删条目、不得追加高赞必选说明。代码实现为常量 `DJ_NOTE_DEFAULT`，`djDefaultNote(){ return DJ_NOTE_DEFAULT; }` 不读任何开关。**用户仍可在输入框里自行改写**，改过的内容优先使用，「↺ 默认」回到这一句。违反本条＝擅自改动站主对外公示口径。 | 站规·已生效 |

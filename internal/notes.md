...
**本仓库是唯一内容源**。任何 AI（DeepSeek / ChatGPT / Claude / Gemini / 任意模型 / 任意新对话）只要拿到站主直发的 Gitee 令牌（见站规「授权与凭据」）就能推送，再完整读完本文件与 `internal/guide.md`，就能完整接手本项目，**无需依赖任何特定对话的记忆**。
...
> - **铁律 · 敏感信息安全（红线）**：① **线上只上传密文 / 干脆不上传**——仓库内**严禁**出现任何明文口令 / 密钥 / API Token / 加密提示；`secrets.json.enc`（AES-256-CBC + PBKDF2 密文）是早期遗留，**已不再作为令牌来源**（2026-08-30 起令牌由站主直发、不入库）。② 解密口令**永不入库**：不得写入 `README` / 任何源码注释 / commit message / hint 文件；口令由维护者**本地记忆**，解密过程**仅在本地**完成。③ 加密脚本与提示文件**不入库**（`secrets-hint.txt`、`scripts/encrypt-secret.sh`、`scripts/decrypt-secret.sh` 等一律不跟踪）。④ 历史上若发生过口令 / 密钥明文入库事故，必须：a) 立即**轮换被暴露的密钥**（旧密钥视为已泄露）；b) **改写 Git 历史**将明文从所有 commit 中抹除并 force-push；c) 在更新记录中留档事故与处置。⑤ 提交前自检：`git grep` 搜索口令、token、key、hint、password 等关键词必须为空。
> - **铁律 · 公告规范**：站内公告须**简洁、凝练、官方口吻**；除「站主杂谈」与「用户要求」类外不得出现「你 / 我 / 我们」等主观语气词；**只发用户可见的站点功能，仓库 / 维护内容（站规、zip 打包、README / 内部文档更新说明等）一律不进公告**；分点公告每条之间**必须换行**（text 内用 `\n`，CSS `white-space:pre-line` 渲染）。
> - **铁律 · 验证纪律（不确定功能 / 新增改动处理流程,2026-08-31 立）**：涉及新增改动或用户未确定、存疑的功能时,所有判断走「先验证、后结论」,禁止把用户说法、模型直觉或表面合理当作事实。适用场景：用户说「X 能/不能」「我觉得应该 Y」「这样会不会 Z」等任何不确定陈述；以及本 AI 想当然给出结论时。
>   1. **假设待验证**：用户提供的解释、诊断、假设、解决方案一律视为「待验证假设」而非既定事实；前提错误或缺乏依据时,明确指出,不顺着说、不默认成立。
>   2. **客观优先证据**：赞同与反对都基于证据与推理,目标是更接近正确结论,不迎合、不回避反例。
>   3. **能验证就验证**：技术判断优先用代码、测试、日志、运行结果或权威文档验证；可执行、可复现、可独立验证的证据优于直觉与表面合理。
>   4. **结论不因人改**：仅在新证据或更强推理出现时才改结论；用户的质疑、信心、重复坚持本身不算新证据。改结论时必须说明具体依据。
>   5. **区分事实 / 假设 / 未知**：明确分开「已验证事实」「当前假设」「未知信息」；证据不足就说明不确定,并给出可操作的验证办法。
>   6. **先查反例**：接受重要判断或方案前,主动检查最强反例、失败场景、边界条件与隐藏假设。
...
├── secrets.json.enc        # 历史遗留：早期令牌密文（OpenSSL 加密），2026-08-30 起不再作为令牌来源
...
- **⚠️ 推送凭据：令牌直发，不入库**（2026-08-30 起，取代原 SSH 方案）：
  - remote 保持 `git@gitee.com:zikang0529/class-site.git`（SSH 地址）不动；**推送时用临时 HTTPS 地址** `https://用户名:<令牌>@gitee.com/zikang0529/class-site.git`，令牌从本地文件读取（`$(cat /root/.codebuddy/artifact/gitee_token_2505)`），**禁止 `git remote set-url`**（令牌会落盘到 `.git/config`）
  - 令牌由站主一次性直发、AI 本地保存复用；**绝不 commit、绝不 push 到远程、不写进任何文档/注释/日志**（见站规「授权与凭据」）
  - **⚠️ 本地令牌文件随沙箱会话结束而消失**，换会话/换 AI 时提醒站主重发
  - 历史方案（SSH 公钥推送，2026-08-29 配置、2026-08-30 弃用）：remote 曾从明文令牌 URL 改为 SSH，公钥 title `class-site-sandbox-20260829` —— **已全部弃用**，仅留档。**不要**再把令牌拼进 URL 存进 `.git/config`（这条仍有效）
...
5. 认证方式：用户名 + Personal Access Token（**GitHub 令牌由站主自行在 GitHub 生成、填进 Gitee 镜像配置，本环境不持有**；曾存于 `secrets.json.enc`，2026-08-30 起该文件已弃用）
...
- **现行方案（2026-08-30 起）：令牌不入库，站主直发、AI 本地复用** —— 见本条第 131 行「站主拍板」与台账站规「授权与凭据」。**Gitee 推送令牌现在只存在于：① 站主备忘录（唯一长期副本）② 当前 AI 沙箱本地文件（临时）**，仓库里什么都没有。
- ~~所有令牌已加密存储在 `secrets.json.enc`~~ —— **历史遗留（2026-08-30 更正）**：该文件是早期「令牌加密入库」方案的残留（Gitee/GitHub 令牌），2026-08-30 起**不再作为任何令牌的来源**，放着不碍事、不用管它。若仍要读：加密算法 AES-256-CBC + PBKDF2（迭代 200000），**密码由仓库维护者保管**（见下条口令纪律）。
- **解密口令仅站主本人知晓，不入库、也不写进任何文档——包括位数也不记录**（2026-08-30 更正，见当日更新记录）：① 文档里写「6 位数字」**本身违反本站站规**（红线明令「严禁明文口令 / 密钥 / 加密提示入仓」），2026-08-27 那次泄露事故的成因之一就是 `secrets-hint.txt` 写了「6位数字」提示，把暴力破解范围直接缩小到 100 万种；② 那个 6 位口令**已在 2026-08-27 轮换作废**，现行口令是站主自定的新口令（**位数 AI 无从得知，也不应得知**），文档里残留的「6 位」是轮换前的旧描述，一直没同步。③ 忘了口令也不要紧——里面的令牌都能去各自平台重新生成，不影响站点运行。
...
- **【2026-08-30 站主拍板】换 AI 不用 SSH 公钥了，改「令牌直发」**：**令牌不加密、不入库**，由站主自己存备忘录，**需要时站主主动发给 AI，发完删聊天记录**。理由见台账站规「授权与凭据」条 —— 简言之：加密入库等于没加密（密文密钥同在仓库），配公钥则换一个 AI 就要重配一次。AI 侧纪律：不主动索要、不写进任何文件、不回显、不用 `git remote set-url`（会落盘）、干完提醒站主删记录。**已被站主明确否决的方案不要再提**：令牌加密存仓库 + 密钥交 AI 解密。**2026-08-30 晚更新（站主原话「这个令牌就存你的本地，每次直接用就行，但是不要上传」）**：令牌已一次性直发并**保存在 AI 本地工作区**（`/root/.codebuddy/artifact/gitee_token_2505`，权限 600，**在仓库外**），之后每次推送直接复用、不再索要；**硬红线：绝不上传**（不入库、不 push、不写进文档）。⚠️ **该文件随沙箱会话结束而消失**，届时或换新 AI 需站主重发；**站主备忘录是唯一长期副本**。
...
- `token: _cpnDec()`——**不明文存储**！完整 key 加密在 `secrets.json.enc` 的 `voicehub.key` 字段（**口令仅站主本人知晓，不入库**），前端只存混淆密文 `_CPN_ENC`（xor+hex），运行时 `_cpnDec()` 解密。
...
1. 明文权威版本只存 `secrets.json.enc`（**解密脚本早已不入库**，需要时用 openssl 现场解密，口令由站主自行输入；**口令值与口令位数都不写进任何文档**）
...
| 日期 | 内容 |
|------|------|
| 2026-09-20 | 🎬 **网页内视频播放能力（B站分享链接嵌入 + 云端/直链原生控件，均在站内不跳转）+ ClassIsland 三周年纪念卡片（用户要求，不进公告）**：**背景**——站主要让纯静态网页也能「在网页里播云端/直链视频」并可分享「B站视频链接」，且**都在站内播放、不跳转到外部站**，带暂停/全屏等基础操作。恰逢班级课表 ClassIsland 昨天（2026-09-19）三周年，要把官方三周年纪念视频放上去庆祝。**落地**：① 新增视频弹窗 `#videoMask`（复用站内 `.allboard-mask{display:none}` + `.show{display:flex}` + 点遮罩关闭机制，与 qrMask 一致）；② 新增 JS `parseVideo(url)` / `openVideo(url,title)` / `closeVideo()`——链接含 `BV` 号（大小写敏感）走 `https://player.bilibili.com/player.html?bvid=XXX&page=1&high_quality=1&danmaku=0&autoplay=0` 的 **iframe 内嵌**（不跳 B站，带「在 B站打开」兜底链接）；否则若是 `http(s)` 直链走 `<video controls playsinline preload="metadata">` **原生控件**（暂停/全屏/画中画浏览器自带，均可用）；非法链接给友好提示；③ 渲染分支新加 `else if(l.video)` 输出「▶ 播放视频」按钮；④ 新增「班级应用」卡片 **id 18「ClassIsland 三周年纪念」**（iconText 🎉，desc 三周年庆祝语，`video` 填官方三周年回顾短片 `https://www.bilibili.com/video/BV1sJe36ZEb9/`）。**⚠️ 踩坑（关键）**：`parseVideo` 原把 BV 号 `.toUpperCase()`，会把 `BV1sJe36ZEb9` 大写成 `BV1SJE36ZEB9`——B站按**大小写敏感**查找，大写后找不到视频，弹窗报「本视频可能…无法正常播放」。经对照实验逐层证伪 SW 拦截 / Referer 防盗链 / `display:none` 容器 0×0 / 注入时序等假设后，锁定真因就是 BV 大写化 bug，删掉大小写转换并加注释警示，修复后端到端实测全部正常（iframe `src=bvid=BV1sJe36ZEb9`，无页面错误）。⑤ `sw.js` 缓存版本 v9→**v10**（清掉旧缓存）。**按用户「不要加公告」指示，站内公告数组未动**——仅此处内部记录；视频庆祝属班级应用内容，依站规「游戏/隐藏内容不进公告」亦合理。**验证**：Playwright 真浏览器实测——B站 iframe 正常加载（弹窗 `display:none` 标准写法即可，无需 `visibility` 时序 hack，已回退非必需改动）、直链 `<video>` 原生控件 `controls/fullscreen/pip` 均 `true`、关闭弹窗清空 `videoBody` 释放资源、无真实页面错误；`node --check` 通过；改前已打 `backup/class-site-backup-before-20260920-1246.zip`。已重打包 `class-site-backup.zip`；已重新发布备用链接（同链接，已 verified）。 | 已上线 |
| 2026-09-07 | 📚🎮 **学习资料两卡片暂停开放（灰显不删数据）+ 飞机大战改经典手绘风（用户两项需求）**：① **学习资料**：id 6「课堂笔记」/ id 7「教学课件」加 `paused:true`——渲染为灰显卡片（`grayscale(1)` + 灰顶边 + 灰底 + 去流光 + 不上浮），不渲染门禁输入框与任何入口按钮，显示「⛔ 因网络问题，暂时关闭，恢复开放时间另行通知」提示条；**gate 密文数据全部保留**，恢复开放时删掉 `paused` 字段即回归，无需重配。站内公告已发（2026-09-07 12:57，【班级通知】，category site）。② **飞机大战**：`games/plane.html` 视觉层整体重做为 2014 微信内置经典「打飞机」手绘铅笔风（站主附参考图）——纸面底纹+灰点飞屑、白色双引擎大飞机（黑线稿+抖动笔触）、黑线稿小敌机三型（受伤冒烟）、橙色小子弹、涂鸦陨石装饰、涂鸦星芒爆炸、尾焰闪烁；实现为固定种子伪随机（mulberry32）+ 离屏预渲染 sprite，**玩法逻辑（生命 3/计分/渐加速/音效/操作）完全不变**。修复一处自引入 bug：陨石 sprite 生成 `for(var rv…)` 闭包共享变量致三变体同形 → 改函数式 `makeRock(v)`。③ **顺手修**：51LA SDK 地址 `//sdk.51.la/…` 改显式 `https://sdk.51.la/…`（协议相对地址在 http 环境被 403 致 `LA is not defined`；https 正常，线上行为不变）。④ `sw.js` 缓存版本 v8→**v9**；README 学习资料行加「（因网络问题暂停开放中）」；重打包 `class-site-backup.zip`；改前已打 `backup/class-site-backup-before-20260907-1301.zip`。**验证**：Node 编译两个文件全部内联 script 块语法 OK；Playwright 实测——飞机大战开局跑 6s 无页面错误（除本地环境 51LA http 403 已修）、学习资料卡片亮/暗双模式截图核对（灰显+提示条可读，暗色提示文字 `var(--fg-3)` 达标）。游戏类改动按站规**不进公告**，仅学习资料暂停开放进公告。 | 已上线 |
| 2026-08-31 | ⚖️ **立「验证纪律」铁律（用户要求，适用于新增改动与不确定功能）**：站主要求以后**任何新增改动**、以及**用户未确定 / 存疑的功能**，所有判断走「先验证、后结论」，禁止把用户说法、模型直觉或表面合理当事实。已写入第 0 节铁律区「铁律 · 验证纪律」6 条：① 用户提供的解释 / 诊断 / 假设 / 解决方案一律视为「待验证假设」而非既定事实，前提错误或缺依据时明确指出；② 赞同与反对都基于证据与推理，不迎合；③ 技术判断优先用代码 / 测试 / 日志 / 运行结果 / 权威文档验证，可复现、可独立验证的证据优于直觉与表面合理；④ 仅新证据或更强推理才改结论，用户的质疑 / 信心 / 重复坚持本身不算证据，改结论须说明具体依据；⑤ 区分「已验证事实 / 当前假设 / 未知信息」，证据不足就说明不确定与验证办法；⑥ 接受重要判断或方案前主动查最强反例、失败场景、边界条件、隐藏假设。对任意 AI（含换模型 / 换对话）生效，经第 0 节「通读站规」铁律约束执行。 | 已生效 |
| 2026-08-31 18:26 | 🗓️ **隐藏「报到」倒计时项（用户要求）**：`index.html` 的 `IMPORTANT_DATES` 中「报到」（2026-08-31 18:00）已从首页「重要日期倒计时」面板与「查看全部」弹窗移除（日期已过，原面板按「天」过滤仍挂负数倒计时）；同步清理无引用的死配置 `SEMESTER.reportDate`。已同步：站内公告（2026-08-31 18:26，category site）。 | 已生效 |
| 2026-08-30 | 🧹 **三文档大扫除：清理全部过时内容 + 立「文档新旧一致性」站规（用户要求）**：站主原话「每次打开都会发现有很多旧的东西还放在里面，比如 ssh 班级排行榜等等……以后更新不仅是更新新的东西，还有看看旧内容和新的有没有冲突，有没有过时，记得及时改」。**README.md**（对照 `index.html` 实际分类逐一核实）：① 「班级事务」去掉**已隐藏的排行榜**与**实为「其他工具」的联系方式查询**，改为「意见反馈、班级活动」；② 「学习资料」去掉**实为「其他工具」占位且未上线的私人云盘**；③ 「其他工具」补「联系方式查询」；④ 班级应用补 EchoMusic。**guide.md**：① 0 节凭据改为「令牌 + 备份口令」两样，SSH 标「已弃用」；② 2.1/2.2 重写为「现在推送方式 = 令牌直发」+「令牌过期/重建」；③ 原 2.4「清理 SSH 公钥」改为 **2.3（历史）SSH 已弃用**，事故教训压缩留档；④ 3.0 工作模式表（SSH 三选一）**精简为「当前只有令牌直发」**；⑤ 3.1 原则改「推代码用令牌」；⑥ 3.5 回收删掉「配 SSH 公钥」步骤；⑦ 4.2 `secrets.json.enc` 标注**历史遗留已弃用**；⑧ 速查表 `Permission denied (publickey)` 行改为 `401/403 令牌失效`；⑨ 第 6 节维护说明加「新旧一致性检查」。**notes.md**：① 第 0 节开头「解密 secrets.json.enc 拿令牌」改为「令牌直发」；② 敏感红线①改「secrets.json.enc 为历史遗留，不再作为令牌来源」；③ 3.1 节推送凭据改为令牌直发口径（SSH 留档）；④ 运维备忘 SSH 条标「历史·已弃用」并提炼教训（沙箱文件随会话消失）；⑤ 台账新增站规「文档新旧一致性」；⑥ 更新记录本条。**对照依据**：`index.html` 分类/卡片现状（排行榜 `active:false` 隐藏、私人云盘 `active:false` 隐藏、联系方式查询 `categoryId:8` 其他工具、班级活动 `comingSoon`）。 |
| 2026-08-30 | 🔑 **令牌授权再升级：本地保存、直接复用、不上传（用户拍板）**：站主原话「这个令牌就存你的本地，每次直接用就行，但是不要上传」。执行：① 令牌保存在本地工作区 `/root/.codebuddy/artifact/gitee_token_2505`（权限 600，**在仓库外**），推送直接复用、不再每次索要；② **上传禁令入站规**——令牌绝不 commit、绝不 push 到远程、不写进任何文档/注释/日志；③ 用 `$(cat …)` 读取令牌拼接临时 HTTPS 地址推送，**不改 `git remote`**；④ 明确标注**沙箱临时性**：文件随会话消失，换会话/换 AI 需站主重发，站主备忘录是唯一长期副本。已同步：`internal/notes.md` 站规「授权与凭据」+ 4.1、`internal/guide.md` 2.5 / 3.0 / 3.2 / 3.3 / 5。本次令牌已实测推送上游成功（`2a4acfb..d0d209b`），全仓残留 0。 |
| 2026-08-30 | 🔐 **学习资料链接隐秘化（用户要求，不进公告）**：站主查看网页源码时发现学习资料板块的**云盘链接与防伪码明文躺在源码里**——Ctrl+U 即可抄走，门禁形同虚设（答案做了哈希加盐，链接却裸奔）。处理：`index.html` 新增 `GATE_SEAL_KEY` / `gateUnseal(b64)`（XOR + Base64）/ `gateUrl(l)` / `gateCode(l)`；id=6 课堂笔记、id=7 教学课件的 `gate.url` / `gate.code` **明文清空**，改存密文 `sUrl` / `sCode`；渲染处与 `copyCode()` 改为取解密后的值（旧数据无密文时回退明文，保证兼容）。验证：`node /tmp/verify_seal.js` 密文往返 **ALL OK**；源码明文残留计数 **0**；`node --check` 通过；新增 `test_gate_seal.py` **19 项全过**（含「源文件无明文」「逐字符解密正确」「通过门禁后页面呈现正确」「copyCode 取到明文」）。**⚠️ 这是混淆不是加密**（密钥同在 JS 中），只挡「随手 Ctrl+U」这一场景，已在站规里写明，不得夸大为加密。 |
| 2026-08-30 | 🔑 **凭据方案定版：令牌不入库、不加密，由站主直发（用户拍板）**：站主原话「干脆什么都不要……我个人令牌我自己储存好，放在备忘录里……需要的话我会把令牌发给 AI……记得要删聊天记录」。据此**废弃**「令牌加密入库 + 密钥交 AI 解密」思路（理由：密文与密钥同在仓库 = 等于没加密；且密钥一发给 AI 就进聊天记录，令牌明文照样暴露），采用**令牌直发**。`internal/guide.md`：新增 2.5 节（三方案对比表 + 操作步骤 + 给 AI 的规矩）、3.0 工作模式表增「③ 令牌直发」、3.2 权限清单改为「主动索要直接拒绝」与「令牌可给但守规矩」两类、3.3 开场白模板新增「【关于凭据】」段、3.5 回收流程首条改为「删聊天记录」、第 5 节速查表增 2 条。本文件：新增两条站规（授权与凭据 / 源码不得出现需隐藏链接）、4.1 口令描述补「由站主直发」口径。 |
| 2026-08-30 | ✅ **「6 位还是 8 位」疑问已澄清——站主没记错**：文档里写的「6 位数字」是 **2026-08-27 口令轮换之前**的旧描述，一直没同步；且**写位数本身就违反站规**——2026-08-27 那次泄露事故的成因之一正是 `secrets-hint.txt` 写了「6 位数字」提示，把暴力破解范围直接缩小到 100 万种。现行口令是站主自定的新口令，**位数 AI 无从得知也不应得知**。已更正本文件 4.1 节与 `internal/guide.md` 4.2 节两处，改为「密码是你自己设的（位数不写进任何文档）」。 |
...
| 2026-08-25 | 导出备份密码输入错误时提示「站主高中绝对不会忘记的数字」；新增 `secrets-hint.txt` 密码提示文件（**曾明文写出解密口令的位数，具体位数此处不再复述**）；主链接决策待定：若 github.io 国内访问稳定则作为主链接并停用 WorkBuddy。 |
...
- **【运维备忘·历史】沙箱 SSH 密钥是临时的（2026-08-30 事故后立，同日晚随 SSH 方案一并弃用）**：~~本环境推送**完全依赖** `~/.ssh/id_ed25519_gitee` 这一对密钥，一旦站主在 Gitee 删掉对应公钥，读写立刻全部 `Permission denied (publickey)`……~~ **⚠️ 本条描述的是已弃用的 SSH 方案**，现在推送走令牌直发（见站规「授权与凭据」），**不再需要配置任何公钥**。保留本条的教训价值：**沙箱里的任何文件/密钥都是临时的、随会话消失**——本地令牌文件同样如此，换会话要提醒站主重发。
- **【站规】授权与凭据：令牌不入库、不上传，站主一次性直发、AI 本地复用（2026-08-30 立，当日两次拍板）**：换 AI 的授权方式定为**「令牌直发」**——**仓库里不放任何令牌、不放密文、不放加密脚本、不放密钥**。站主比较过三个方案后定的：①「每个 AI 单独配 SSH 公钥」→ 换一个 AI 就要重配一次，**太麻烦**；②「令牌加密存进仓库、密钥交给 AI 解密」→ **等于没加密**（密文和密钥同在仓库里，拿到仓库就能当场解开，只比明文多一层纸），且**密钥一发给 AI 就进了聊天记录**，令牌明文照样暴露；③「令牌自己存备忘录，需要时发给 AI」→ ✅ **采用**。**2026-08-30 晚站主进一步拍板（原话「这个令牌就存你的本地，每次直接用就行，但是不要上传」）**：令牌**一次性直发**后，AI 保存在**本地工作区文件（仓库外）**，之后每次推送**直接复用，不再每次索要**。**执行口径**：① **上传禁令（硬红线）**——令牌**绝不** commit 进仓库、**绝不** push 到任何远程、不写进 README / 文档 / 注释 / `commit message` / 日志；② **不回显**（连「前四位是 xxxx」都不许说）；③ 推送用临时 HTTPS 地址（`git push https://用户名:令牌@gitee.com/zikang0529/class-site.git master`，令牌可从本地文件 `$(cat …)` 读取），**禁止 `git remote set-url` 写进 `.git/config`**（那会让令牌落到磁盘上）；④ **⚠️ 沙箱临时性**——本地文件随沙箱会话结束而消失，**届时或换新 AI 时需要站主再发一次**；**站主备忘录始终是唯一长期副本**；⑤ 干完活**主动提醒站主删聊天记录**（一次性直发的那条）。**同时**：账号密码、点歌 API key 明文、备份包解压口令（**连位数也不许问、不许记**）一律不给 AI。操作步骤与开场白模板见 `internal/guide.md` 2.4 / 3.0 / 3.3。
- **【站规】源码里不得出现需隐藏的链接与访问码（2026-08-30 立，站主原话「我去查看网页源文件的时候，都不要透露出任何一个密钥，或者说我让你隐藏的一些链接」）**：① 学习资料等**需隐藏**板块的云盘链接与防伪码，**不得以明文出现在源码**——现改为 `sUrl` / `sCode` 密文（XOR + Base64）+ `gateUnseal()` 运行时还原，原明文字段 `url` / `code` **已清空**；② **诚实说明：这是混淆不是加密** —— 密钥 `GATE_SEAL_KEY` 同在 JS 里，认真翻就能解出来。它解决的是「同学随手 Ctrl+U 抄走链接」这一真实场景，**不是**防专业攻击，**不得在文档里把它吹成加密**；③ 门禁**答案**走的仍是 `hashSecret(normAnswer(v), GATE_SALT)` 哈希加盐，那个才是真不可逆——**两者性质不同，别混为一谈**；④ 后续新增需隐藏的链接 / 口令，**一律走 `gateUnseal` 密文字段**，不要再发明新的明文字段；⑤ 曾评估过「用门禁答案派生解密密钥」，**没采用**：现有答案（男女人数之比）空间仅约 10 种会被秒破，且答案本身可能出现在页面上。
...
- **【站规】文档新旧一致性（2026-08-30 立，站主原话「每次打开都会发现有很多旧的东西还放在里面……以后更新不仅是更新新的东西，还有看看旧内容和新的有没有冲突，有没有过时，记得及时改」）**：**每次改动（无论改代码还是改文档）前后，必须扫一遍 `README.md` / `internal/notes.md` / `internal/guide.md` 全文**，检查：① 与站点现状冲突的旧描述（例：已弃用的 SSH 推送、已隐藏的排行榜入口、已下线的板块）→ **当场改掉**，不允许「下次再说」；② 被新方案取代的旧章节 → 标注「已弃用 / 历史」或直接精简，**不得把已废弃的操作继续写成「现行方法」**；③ 操作教程（guide.md）里的步骤必须能照做成功，过时命令/路径不留。**分类原则**：技术历史（事故、弃用原因）留在 `notes.md` 台账；`guide.md` 只写当前有效操作，历史用「（历史）」小节一句话带过；`README.md` 只写站点当前实际功能，**不写占位/隐藏/已隐藏的功能**。
...
- **【站规】多 AI 协作同步（2026-08-30 立，站主问「A 和 B 都改仓库，切回 A 会不会对不上」后立）**：**远端仓库是唯一真相源，每个 AI 本地只是副本**。站主可能同时给多个 AI（A、B……）改本仓库。**纪律**：① **动手前必拉**：`git fetch <HTTPS带令牌地址> master` → `git rebase FETCH_HEAD`，把别的 AI 推的新提交接在本地改动之前，再编辑、再提交；② **绝对禁止 `git push --force`**——那会把别的 AI 的提交整个覆盖，数据直接没了；推送被拒（`! [rejected] (fetch first)`）是正常保护，先 fetch+rebase 再看；③ **同会话比跨会话危险**：会话还活着时本地文件是某天别的 AI 推送前的旧版，不拉就改会撞合并冲突；**新开对话工作区自动重新拉取、反而是最新的**；④ 推送前用 `git status -sb` 确认无分叉。该纪律同步写入 `internal/guide.md` 第 1 节。
- **【运维备忘】备用链接（WorkBuddy 沙箱发布）是沙箱派生的，非仓库自动同步（2026-08-30 记）**：当前备用链接 `https://a5048c773a210b3d4-25579.app.workbuddy.link/` 来自 A 沙箱目录 `/workspace/.publish-class-site/` 的发布，**与仓库无自动关联**——B 推了改动它不会自动变，且 A 会话结束沙箱消失后该链接即失效/停更。站主 2026-08-30 决定：**暂不改为仓库托管**（Gitee Pages `zikang0529.gitee.io/class-site/` 或 Vercel 自动部署），维持现状。同步靠人工流程：**站主若让 B 推了改动并通知 A「git 下来」，A 须先 `git fetch`+`rebase` 拉最新，再重新发布备用链接**，否则备用链会滞后于主链。主链 GitHub Pages 仍是全自动真相源，不受影响。若日后想让备用链也仓库化，步骤见 `internal/guide.md` 第 1 节多 AI 铁律注释。
- **【运维备忘】沙箱空闲自动同步定时任务（2026-08-30 立）**：为补「A 会话还活着但闲置 >24h、B 却推了」的空档，建了系统级 cron——每 6 小时跑 `/root/.codebuddy/artifact/sync_if_idle.sh`：① 工作树有未提交改动则跳过（不覆盖进行中工作）；② 沙箱 24h 内有改动则跳过；③ 远端领先才 `git fetch`+`git rebase FETCH_HEAD` 接上最新，否则 no-op；④ 令牌从本地文件读，不写 `.git/config`。日志在 `/root/.codebuddy/artifact/sync_if_idle.log`。**限制（重要）**：cron 装在沙箱内，随沙箱重建/会话结束而失效——它只覆盖「A 会话存活但闲置」这一种情况；若 A 会话已死，新会话会重新拉取（自动最新），不用它管。真正跨会话永不同步仍需把备用链改为 Gitee Pages 仓库托管。
- **【运维备忘】index.html 含 51LA 统计代码（2026-08-30 立）**：已接入 51LA 匿名访问统计（PV/UV/来源/设备/地区），代码在 `index.html` 末尾 `</body>` 前（搜 `LA_COLLECT` 即定位），**主链与备用链部署同一份文件，故一个统计 ID 同时覆盖两个入口**。**换 AI 接手切勿误删这段**——它看着像普通第三方脚本，删了统计即断。**`screenRecord`（屏幕录制）已主动关闭**：统计人数不需要录屏，录制同学屏幕涉隐私，与「对用户诚实」站规冲突。**隐私说明在站内公告 2026-08-30 23:36 那条**，重写公告勿删。
...
| 日期 | 用户要求 | 状态 |
|------|----------|------|
| 2026-09-20 | **网页支持在站内播放视频（B站分享链接 + 云端/直链，均不跳转外部站）+ 放 ClassIsland 三周年纪念视频（用户要求，不进公告）**：① 原话「让静态网页支持在网页播放云端视频，可以通过云链或者哔站分享的链接。不要加公告。是我们的那个班级应用里的那个班级课表 classisland 的，昨天也就是 9 月 19 号刚好三周年，我想把它发布的 3 周年纪念的视频放上去，也庆祝一下」——落地：网页内视频播放弹窗（B站 `player.bilibili.com` iframe 内嵌 + 云端/直链 `<video>` 原生控件，均在站内播放不跳转，带暂停/全屏/画中画）；② 澄清（用户）「这次用的是我发的 B站视频链接，同学在网页里直接看、不需要跳转到 B站；后面可能还会发云端或直链，也是在网页内部不跳转，可以有暂停、全屏等基础操作」；③ 选定视频（用户拍板「用 BV1sJe36ZEb9」）——经 B站 API 核实为官方三周年回顾短片《大屏课表软件 ClassIsland 三周年回顾短片：以此为始，携手向前》（UP HelloWRC_dev，2026-09-19 12:00 发布，49 秒），已填进「班级应用」卡片 id 18。④ **明确约束：不要加公告**（站内公告数组未动，仅本台账 + 第 9 节记录）。 | 已上线 |
| 2026-09-07 | **学习资料两卡片暂停开放（灰显、不删数据）+ 飞机大战改回 2014 微信经典手绘风（用户要求）**：① 原话「暂时关闭学习资料对应的里面两个卡片的进入入口，原因就说是由于网络问题，暂不对其进行开放。不用删除，只是先把两个渐变就是改为灰色」——落地：`paused:true` 灰显 + 关入口 + 提示条，gate 密文保留，**恢复时删掉 paused 字段即回归**；② 原话「小游戏里面的那个飞机大战的话，可以改回以前那种。就是在14年左右微信内置的那种小游戏类似图片这种，会更好玩一点」——plane.html 重做为经典打飞机手绘铅笔风（附图参考），玩法不变。**恢复学习资料开放时：删 id 6/7 的 `paused: true` + 更新公告/README/本记录。** | 已上线 |
| 2026-08-30 | **接入 51LA 匿名访问统计（用户要求）**：`index.html` 末尾 `</body>` 前插入 51LA SDK（`LA_COLLECT`+`LA.init`），开 `autoTrack`+`hashMode`（适配单页 PWA 翻页），**主动关闭 `screenRecord` 屏幕录制**（统计人数不需要录屏，且录同学屏幕涉隐私，与「对用户诚实」站规冲突）。主链/备用链同源同文件，故**一个 ID 同时统计两个入口**，后台可按域名拆分对比。隐私说明已写进站内公告（2026-08-30 23:36）。换 AI 接手勿误删该段（见下方运维备忘）。 | 已上线 |

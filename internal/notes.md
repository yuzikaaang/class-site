...
- **【2026-08-30 站主拍板】换 AI 不用 SSH 公钥了，改「令牌直发」**：**令牌不加密、不入库**，由站主自己存备忘录，**需要时站主主动发给 AI，发完删聊天记录**。理由见台账站规「授权与凭据」条 —— 简言之：加密入库等于没加密（密文密钥同在仓库），配公钥则换一个 AI 就要重配一次。AI 侧纪律：不主动索要、不写进任何文件、不回显、不用 `git remote set-url`（会落盘）、干完提醒站主删记录。**已被站主明确否决的方案不要再提**：令牌加密存仓库 + 密钥交 AI 解密。
...
- **【站规】授权与凭据：令牌不入库、不加密，由站主直发（2026-08-30 立，站主拍板「就用这一版方案」）**：换 AI 的授权方式定为**「令牌直发」**——**仓库里不放任何令牌、不放密文、不放加密脚本、不放密钥**。站主比较过三个方案后定的：①「每个 AI 单独配 SSH 公钥」→ 换一个 AI 就要重配一次，**太麻烦**；②「令牌加密存进仓库、密钥交给 AI 解密」→ **等于没加密**（密文和密钥同在仓库里，拿到仓库就能当场解开，只比明文多一层纸），且**密钥一发给 AI 就进了聊天记录**，令牌明文照样暴露；③「令牌自己存备忘录，需要时发给 AI」→ ✅ **采用**。**执行口径**：令牌由站主自存，**站主主动发、AI 不得主动索要**；AI 收到后只在当前会话命令行使用，**不写进任何文件 / 注释 / `commit message` / 日志**，**不回显**（连「前四位是 xxxx」都不许说）；推送用临时 HTTPS 地址（`git push https://用户名:令牌@gitee.com/zikang0529/class-site.git master`），**禁止 `git remote set-url` 写进 `.git/config`**（那会让令牌落到磁盘上）；干完活**主动提醒站主删聊天记录**。**同时**：账号密码、点歌 API key 明文、备份包解压口令（**连位数也不许问、不许记**）一律不给 AI。操作步骤与开场白模板见 `internal/guide.md` 2.5 / 3.0 / 3.3。

...
| 日期 | 内容 |
|------|------|
| 2026-08-27 | 🎟️ **四款游戏周常点歌券 + 涂鸦跳跃平台修复**：① 贪吃蛇（60 豆，文案 100→60）、像素飞鸟（40 管）、俄罗斯方块（120 分）、涂鸦跳跃（3000 高）达标各发一张点歌券；限领由「每日」改为「**每周一张、每周一刷新**」（`weekKey()` 周一起点 + `song_coupon_w_<game>`，四游戏独立计算）；领券弹窗/复制/发券逻辑复制自贪吃蛇（`claimCoupon` 调 VoiceHub card-codes，旧 key 混淆密文）。② 涂鸦跳跃平台生成修复：初始与新增平台 x 均限制在上一平台左右 60~160px（`djNextPlatX`，跳远极限约 200px），消除「一个最左一个最右跳不过去」。 |
...
| 2026-08-27 | **抽歌排期（VoiceHub 拉歌 + 本地排期）**：点歌平台卡片图标连点 5 次 → 本地身份验证（身份证后四位 + 户籍地村名，答案 xor 混淆存前端、不上传）→ 抽歌面板。参数：投稿时间范围（快捷 全部/近3/近7/近30 天 + 自定义 datetime-local——**「全部」= 不限时间**）、抽取数量、**高赞必选 N 首（点赞最高前 N 必定中选，🔥 标记）**、排除已播放/已排期。流程：`djFetchSongs()` 分页拉取 `/api/open/songs`（新只读 key，xor 混淆密文）→ 本地按 `createdAt` 过滤 → 必选+随机抽签 → 存入本地 `cls_dj_schedule`（批次记录）→ 展示 + 一键复制公示文本。验证接入全站自锁（scope 'dj'，连错 4 次锁 30s 递增），输入为密码型+小眼睛防偷看。面板内容可滚动。VoiceHub 开放 API 无排期写接口，按用户选定采用**本地排期**。 | 已上线 |

...
| 2026-08-27 | **抽歌排期（VoiceHub 拉歌 + 本地排期）**：点歌平台卡片图标连点 5 次 → 本地身份验证（身份证后四位 + 户籍地村名，答案 xor 混淆存前端、不上传）→ 抽歌面板。参数：投稿时间范围（快捷 全部/近3/近7/近30 天 + 自定义 datetime-local + 「不限时间」开关）、抽取数量、高赞优先（按 voteCount 加权）、**高赞必选 N 首（点赞最高前 N 必定中选，🔥 标记）**、排除已播放。流程：`djFetchSongs()` 分页拉取 `/api/open/songs`（新只读 key，xor 混淆密文）→ 本地按 `createdAt` 过滤 → 必选+随机/加权抽签 → 存入本地 `cls_dj_schedule` → 展示 + 一键复制公示文本。验证接入全站自锁（scope 'dj'，连错 4 次锁 30s 递增），输入为密码型+小眼睛防偷看。VoiceHub 开放 API 无排期写接口，按用户选定采用**本地排期**。 | 已上线 |

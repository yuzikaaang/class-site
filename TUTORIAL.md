...
| 2026-08-28 | **新增已备案备用链接（Gitee Pages）**：主链接为 GitHub Pages（`yuzikaaang.github.io/class-site/`，未备案、微信拦截）；新增 Gitee Pages 备用 `https://zikang0529.gitee.io/class-site/`（OSChina 已备案、微信可打开），填入 `SITE_DATA.siteLinks` 备用项并同步站内公告。站主需在 Gitee 仓库开启 Pages（master 根目录）后生效。原 WorkBuddy Artifact（403）弃用。 | 处理中（待站主开启 Gitee Pages） |

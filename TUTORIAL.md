...
> - **铁律 · 公告规范**：站内公告文案须**简洁、凝练、官方口吻**；除「站主杂谈」板块与「用户要求」类公告外，不得出现「你 / 我 / 我们」等主观语气词，表达力求短而完整。
...
| 日期 | 内容 |
|------|------|
| 2026-08-26 | 🧹 **数据管理新增「全部清理」 + 立站规·公告规范**：① 侧边栏「💾 数据管理」新增 **「全部清理（恢复首次打开）」** 按钮，新函数 `wipeAllData()` 执行 `localStorage.clear()` + `sessionStorage.clear()` 后刷新，等同于首次打开（清空游戏记录、班级签、点歌券、主题/音效、所有密码与暗号的 `until` 锁状态、设备标识 `cls_device_id` 等全部本机数据）；原 `resetData()`（仅清游戏 key）保留。② 新站规**「公告规范」**：站内公告须简洁凝练官方口吻，除「站主杂谈」与「用户要求」类外禁用「你/我/我们」主观语气词。已记入 TUTORIAL 铁律 + 用户要求台账（站规表）+ README 维护须知 + 站内公告（一条简洁官方公告，无主观语气词）+ 重打包 `class-site-backup.zip`。 |
...
- **令牌不明文**：所有 API / 令牌一律不明文进仓库（VoiceHub Key 存 `secrets.json.enc`，密码 `本地口令` 解密；前端用混淆密文运行时解密）。
...
- **公告规范**：站内公告须简洁、凝练、官方口吻；除「站主杂谈」与「用户要求」类外，禁用「你/我/我们」等主观语气词，短而完整。

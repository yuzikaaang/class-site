...
| 日期 | 用户要求 | 状态 |
|------|----------|------|
| 2026-08-26 | **【站规】用户要求全量留档与遵守**：① 用户提出的**所有要求**写入本「用户要求台账」；② README 顶部加「维护须知」提示此规；③ 每次改动前**不得违反或跳过任何已记录的用户要求**；④ 若当前需求与过往要求矛盾，**先询问用户并同步更新规则**，不得擅自取舍；⑤ 用户每次提出要求或需求变更后，**立即 commit + push 到 Gitee**。 | 生效 |

...
| 日期 | 内容 |
|------|------|
| 2026-08-27 | 🎟️ **四款游戏「已领过再达标」暂停提示 + 飞鸟无敌 + 歌单管理与券管理**：① 四款游戏达标但**本周已领过**时改为**暂停游戏** + 弹窗「本周已领过」+「继续游戏 / 退出游戏」选择（snake `paused`+`claimContinue/claimExit`；bird/tetris/doodle `pauseForClaimed` 等；未领过仍走原领券弹窗）；② **像素飞鸟开局 2 秒无敌**（`invince`，无敌期不判碰撞、鸟半透明闪烁、上下边界夹紧）；③ 抽歌面板新增「🎼 歌单管理」（`djOpenSongs/renderDjSongs`）：先调 API 列出全部歌曲，勾选「排除」即不参与抽签（`cls_dj_excluded`，`djRun` 过滤跳过，持久化可取消）；④ 隐藏空间新增「🎟️ 点歌券管理」（`openCouponAdmin`）：添加（数量+备注，真实 POST 发券）/ 列表（GET 展示 券码/状态/备注/时间）/ 本地禁用（`cls_coupon_disabled` 划线）/ 本地隐藏（`cls_coupon_hidden` 可一键恢复）；实测开放 API DELETE 403、PATCH 404，真实作废需平台后台。⑤ 确认：小游戏发券 `note` 备注已含来源（贪吃蛇吃豆60奖励等）。均属隐藏功能/站主管理，不进公告。 |
...
| 2026-08-27 | **点歌券管理入口（站主）**：隐藏空间新增「🎟️ 点歌券管理」卡片（`openCouponAdmin`）：① **添加**——数量+备注，真实调 POST `/card-codes` 生成（旧发券 key xor 混淆密文 `CP_ADMIN_KEY_ENC`）；② **列表**——GET `/card-codes` 展示券码/状态/备注/时间；③ **禁用**——本地标记 `cls_coupon_disabled`（划线+「已禁用」，可恢复）；④ **删除**——本地隐藏 `cls_coupon_hidden`（列表不显示，可一键恢复全部）。注：开放 API 实测 DELETE 403 无权限、PATCH 404 无禁用路由，**真实作废需到 VoiceHub 平台后台**，本地标记仅影响本站展示。 | 已上线 |
